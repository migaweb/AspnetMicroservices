...
namespace Ordering.Application.Features.Orders.Commands.CheckoutOrder
{
  public class CheckoutOrderCommandHandler : IRequestHandler<CheckoutOrderCommand, int>
  {
    ...
    public async Task<int> Handle(CheckoutOrderCommand request, CancellationToken cancellationToken)
    {
      ...
      _logger.LogInformation($"Order {createdOrder.Id} was successfully created.");
      await SendEmail(createdOrder);

      return createdOrder.Id;
    }

    private async Task SendEmail(Order createdOrder)
    {
      var email = new Email { 
        To = "miga02@gmail.com",
      ...
      {
        _logger.LogError($"Order {createdOrder.Id} failed to send order confirmation: {ex.Message}");
      }
    }
  }
}
